...
async function handleGenerateLevels(request, response) {
  ...
  try {
    ...
    if (!validation.ok) {
      console.warn("Rejected Anthropic level pack:", validation.reason);
...
async function handleTts(request, response) {
  ...
  try {
    ...
    const upstream = await fetch(
      `${ELEVENLABS_API_URL}/${encodeURIComponent(ELEVENLABS_VOICE_ID)}/stream?output_format=${encodeURIComponent(ELEVENLABS_OUTPUT_FORMAT)}`,
      {
        method: "POST",
        headers: {
