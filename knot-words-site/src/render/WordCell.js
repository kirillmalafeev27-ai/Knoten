...
export function drawWordCell(ctx, {
  ...
  const isEndpoint = cell.isStart;
  const pathColor = highlightColor || "#DCE8F8";
  ...
  const frameColor = invalid
    ? "#FF3A5C"
    : occupied || highlightColor || complete
      ? pathColor
      : "#DCE8F8";
  ...
  const orbRadius = isEndpoint ? cellSize * 0.162 : cellSize * 0.055;
  const orbColor = isEndpoint ? cell.color : frameColor;
  const orbFillAlpha = isEndpoint ? (complete ? 0.96 : 0.88) : orbAlpha;
  ctx.beginPath();
  ctx.arc(0, 0, orbRadius, 0, Math.PI * 2);
  ctx.fillStyle = toRgba(orbColor, orbFillAlpha);
  ctx.shadowBlur = isEndpoint ? cellSize * 0.34 : occupied || highlightColor || complete ? cellSize * 0.22 : 0;
  ctx.shadowColor = isEndpoint ? cell.color : occupied || highlightColor || complete ? frameColor : "transparent";
  ctx.fill();
  ctx.shadowBlur = 0;

  if (cell.isStart) {
    ctx.beginPath();
    ctx.arc(half - cellSize * 0.095, -half + cellSize * 0.095, cellSize * 0.05, 0, Math.PI * 2);
    ctx.fillStyle = toRgba(cell.color, complete ? 0.95 : 0.82);
