...
export class Renderer {
  ...
  drawPaths(state) {
    Object.values(state.paths).forEach((pathState) => {
      this.pathDrawer.drawPath(this.ctx, pathState.cells, pathState.color, this.cellSize, {
        active: state.active.sentenceId === pathState.sentenceId && state.active.dragging,
      });
    });
  }
  ...
  drawCursor(state) {
    ...
    if (!activeSentenceId || !state.active.dragging) {
      return;
    }

    const pathState = state.paths[activeSentenceId];
    const lastCell = pathState.cells.at(-1);
    this.pathDrawer.drawCursor(this.ctx, lastCell, pathState.color, this.cellSize, this.time);
  }
  ...
  drawWords(state, timestamp) {
    ...
    state.level.orderedCells.forEach((cell, index) => {
      const pathState = state.paths[cell.sentenceId];
      const inPath = pathState.cells.some((entry) => entry.key === cell.key);
      const activeSentence = state.active.sentenceId === cell.sentenceId;
      const activeCell = activeSentence && pathState.cells.at(-1)?.key === cell.key;
      ...
      const completionAge = pathState.complete ? timestamp - pathState.completedAt - cell.indexInSentence * 35 : Infinity;
