...
export class Renderer {
  ...
  frame(timestamp) {
    ...
    const levelRows = state.level.rows ?? state.level.size;
    const levelCols = state.level.cols ?? state.level.size;
    if (state.level.id !== this.levelId || levelRows !== this.levelRows || levelCols !== this.levelCols) {
      this.#updateGridMetrics(state.level);
    }
  ...
  drawGrid(level) {
    ...
    for (let index = 0; index <= cols; index += 1) {
      ...
      ctx.lineTo(this.offsetX + index * this.cellSize, this.offsetY + rows * this.cellSize);
      ctx.stroke();
    }
    ...
    for (let index = 0; index <= rows; index += 1) {
      ...
      ctx.lineTo(this.offsetX + cols * this.cellSize, this.offsetY + index * this.cellSize);
      ctx.stroke();
    }
  ...
  #updateGridMetrics(level) {
    ...
    this.levelRows = level.rows ?? level.size;
    this.levelCols = level.cols ?? level.size;
    this.cellSize = Math.min(this.width / this.levelCols, this.height / this.levelRows);
    this.offsetX = (this.width - this.cellSize * this.levelCols) / 2;
    this.offsetY = (this.height - this.cellSize * this.levelRows) / 2;
  }
}
