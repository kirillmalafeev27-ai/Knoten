import { SoundEngine } from "./audio/SoundEngine.js?v=20260321b";
import { TTSPlayer } from "./audio/TTSPlayer.js?v=20260321b";
import { GameEngine } from "./core/GameEngine.js?v=20260321b";
import { Renderer } from "./render/Renderer.js?v=20260321b";
import { AnthropicLevelService } from "./services/AnthropicLevelService.js?v=20260321b";
import { HUD } from "./ui/HUD.js?v=20260321b";
import { HintTooltip } from "./ui/HintTooltip.js?v=20260321b";
import { ProfileScreen } from "./ui/ProfileScreen.js?v=20260321b";
import { WinModal } from "./ui/WinModal.js?v=20260321b";
...
class App {
  ...
  init() {
    ...
    this.winModal.setHandlers({
      ...
      onListen: (text) => this.tts.playImmediate(text, { channel: "drag" }),
    });
    ...
    this.game = new GameEngine({
      ...
      callbacks: {
        ...
        onPathEvent: ({ type, cell, sentence, pathState }) => {
          const words = pathState.cells.map((entry) => entry.word);

          if (type === "start") {
            this.sound.play("connect");
            this.renderer.emitAtCell(cell, pathState.color, 5);
            this.tts.startBufferedTrack("drag", words);
            this.tts.updateBufferedTrack("drag", words);
            return;
          }
          ...
          if (type === "complete") {
            ...
            this.tts.updateBufferedTrack("drag", words, { flush: true });
          }
        },
        onCellReplay: ({ cell }) => {
          this.tts.speak(cell.word, {
            ...this.#getCellSpeechContext(cell),
          });
        },
  ...
  #bindCanvas() {
    ...
    const releasePointer = (event) => {
      ...
      this.tts.flushBufferedTrack("drag");
    };
    ...
    this.canvas.addEventListener("contextmenu", (event) => {
      ...
      this.sound.init();
      this.tts.unlock();
      this.tts.playImmediate(cell.word, {
        channel: "drag",
        ...this.#getCellSpeechContext(cell),
      });
    });
  ...
  #getCellSpeechContext(cell) {
    ...
    const previousText = sentence.tokens.slice(0, cell.indexInSentence).join(" ");
    const nextText = sentence.tokens.slice(cell.indexInSentence + 1).join(" ");
    return {
      previousText,
      nextText,
    };
