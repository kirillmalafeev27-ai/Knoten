...
- ElevenLabs TTS proxy for natural German speech with server-side voice/key configuration.
...
4. In the Render environment, set:
   ...
   - Optional: `ELEVENLABS_STABILITY`
   - Optional: `ELEVENLABS_SIMILARITY_BOOST`
   - Optional: `ELEVENLABS_STYLE`
   - Optional: `ELEVENLABS_SPEED`
   - Optional: `ELEVENLABS_USE_SPEAKER_BOOST`
...
The frontend now prefers the same-origin endpoints `/api/levels` and `/api/tts`. That means:
...
The server calls Anthropic's Messages API with the `x-api-key` and `anthropic-version` headers and returns only validated JSON level data. Levels are generated individually so one bad model response no longer forces the whole pack back to fallback content.

For speech, the server proxies ElevenLabs through `/api/tts`, so the browser never sees your ElevenLabs secret. By default the project now uses `eleven_turbo_v2_5` as the model and keeps natural voice settings close to ElevenLabs guidance: stability around `0.5`, similarity around `0.75`, style `0`, and speed `1.0`.
